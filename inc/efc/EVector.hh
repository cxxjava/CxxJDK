...
template<typename T>
class EVector<T*>: public EArrayList<T*>, public ESynchronizeable {
public:
	typedef T* E;
...
template<typename T>
class EVector<sp<T> >: public EArrayList<sp<T> >, public ESynchronizeable {
public:
	typedef sp<T> E;
...
public:
	 ...
	 * @see     Object#equals(Object)
	 */
	virtual int indexOf(T* o, int index = 0) {
		SYNC_IF(_threadSafe) {
			if (index < 0 || index >= size()) {
				throw EINDEXOUTOFBOUNDSEXCEPTION;
			}
	 ...
	 *         than or equal to the current size of this vector
	 */
	virtual int lastIndexOf(T* o, int index = -1) {
		SYNC_IF(_threadSafe) {
			if (size() == 0) {
				return -1;
			}
	 ...
	 *          vector; {@code false} otherwise.
	 */
	virtual boolean removeElement(T* obj) {
		SYNC_IF(_threadSafe) {
			return remove(obj);
		}}
	}
