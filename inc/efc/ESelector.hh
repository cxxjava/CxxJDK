...
public:
	...
	static ESelector* getTemporarySelector() THROWS(EIOException);
	static void releaseTemporarySelector(ESelector* sel) THROWS(EIOException);
...
protected:
	...
	friend class EPollArrayWrapper;
	...
	int doSelect(llong timeout) THROWS(EIOException);
    void processDeregisterQueue() THROWS(EIOException);
    void implDereg(ESelectionKey* ski) THROWS(EIOException);
	...
	void putEventOps(ESelectionKey* sk, int ops);
...
private:
	...
	ELock* keysLock_;         //#added by cxxjava

	// Reference views of the key sets
	ESet<ESelectionKey*>* selectedKeys_; // Removal allowed, but not addition
	ELock* selectedKeysLock_; //#added by cxxjava
    ESet<ESelectionKey*>* cancelledKeys_;// = new HashSet();
    ELock* cancelledKeysLock_; //#added by cxxjava

	// Maps from file descriptors to keys
	EHashMap<EInteger*, ESelectionKey*>* fdToKey_;

	// True if this Selector has been closed
	boolean closed_;// = false;

	// Lock for interrupt triggering and clearing
	ELock* interruptLock_;
