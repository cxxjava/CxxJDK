...
/**
 * @defgroup es_ring Ring Macro Implementations
 * @ingroup ES 
 ...
 * <pre>
 *      struct my_element_t {
 *          ES_RING_ENTRY(my_element_t) link;
 *          int foo;
 *          char *bar;
 *      };
 * </pre>
 ...
 * than one ES_RING_ENTRY field. Each ES_RING_ENTRY has a corresponding
 * ES_RING_HEAD declaration.
 *
 * @warning For strict C standards compliance you should put the ES_RING_ENTRY
 ...
 * <pre>
 *      ES_RING_HEAD(my_ring_t, my_element_t);
 ...
 * to the start of each element, not to the ES_RING_ENTRY structure.
 *
 * <pre>
 *     +->+------+<-+  +->+------+<-+  +->+------+<-+
 ...
 * </pre>
 *
 * The ES_RING_HEAD is nothing but a bare ES_RING_ENTRY. The prev
 ...
 * name for its ES_RING_ENTRY in each element, and has its own type
 * for its ES_RING_HEAD.
 *
 * Note also that if the offset is non-zero (which is required if an
 * element has more than one ES_RING_ENTRY), the unreality of the
 * sentinel may have bad implications on very perverse implementations
 * of C -- see the warning in ES_RING_ENTRY.
 *
 * @param hp   The head of the ring
 * @param elem The name of the element struct
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_SENTINEL(hp, elem, link)				\
    (struct elem *)((char *)(hp) - ES_OFFSETOF(struct elem, link))

/**
 * The first element of the ring
 * @param hp   The head of the ring
 */
#define ES_RING_FIRST(hp)	(hp)->next
/**
 * The last element of the ring
 * @param hp   The head of the ring
 */
#define ES_RING_LAST(hp)	(hp)->prev
/**
 * The next element in the ring
 * @param ep   The current element
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_NEXT(ep, link)	(ep)->link.next
/**
 * The previous element in the ring
 * @param ep   The current element
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_PREV(ep, link)	(ep)->link.prev
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_INIT(hp, elem, link) do {				\
	ES_RING_FIRST((hp)) = ES_RING_SENTINEL((hp), elem, link);	\
	ES_RING_LAST((hp))  = ES_RING_SENTINEL((hp), elem, link);	\
    } while (0)
 ...
 */
#define ES_RING_EMPTY(hp, elem, link)					\
    (ES_RING_FIRST((hp)) == ES_RING_SENTINEL((hp), elem, link))
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_ELEM_INIT(ep, link) do {				\
	ES_RING_NEXT((ep), link) = (ep);				\
	ES_RING_PREV((ep), link) = (ep);				\
    } while (0)
 ...
 *   empty rings... see ES_RING_SPLICE_HEAD for one that does
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_SPLICE_BEFORE(lep, ep1, epN, link) do {		\
	ES_RING_NEXT((epN), link) = (lep);				\
	ES_RING_PREV((ep1), link) = ES_RING_PREV((lep), link);	\
	ES_RING_NEXT(ES_RING_PREV((lep), link), link) = (ep1);	\
	ES_RING_PREV((lep), link) = (epN);				\
    } while (0)
 ...
 *   empty rings... see ES_RING_SPLICE_TAIL for one that does
 * @param lep  Element in the ring to splice after
 * @param ep1  First element in the sequence to splice in
 * @param epN  Last element in the sequence to splice in
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_SPLICE_AFTER(lep, ep1, epN, link) do {			\
	ES_RING_PREV((ep1), link) = (lep);				\
	ES_RING_NEXT((epN), link) = ES_RING_NEXT((lep), link);	\
	ES_RING_PREV(ES_RING_NEXT((lep), link), link) = (epN);	\
	ES_RING_NEXT((lep), link) = (ep1);				\
    } while (0)
 ...
 *   empty rings... see ES_RING_INSERT_HEAD for one that does
 * @param lep  Element in the ring to insert before
 * @param nep  Element to insert
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_INSERT_BEFORE(lep, nep, link)				\
	ES_RING_SPLICE_BEFORE((lep), (nep), (nep), link)
 ...
 *   empty rings... see ES_RING_INSERT_TAIL for one that does
 * @param lep  Element in the ring to insert after
 * @param nep  Element to insert
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_INSERT_AFTER(lep, nep, link)				\
	ES_RING_SPLICE_AFTER((lep), (nep), (nep), link)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_SPLICE_HEAD(hp, ep1, epN, elem, link)			\
	ES_RING_SPLICE_AFTER(ES_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_SPLICE_TAIL(hp, ep1, epN, elem, link)			\
	ES_RING_SPLICE_BEFORE(ES_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_INSERT_HEAD(hp, nep, elem, link)			\
	ES_RING_SPLICE_HEAD((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_INSERT_TAIL(hp, nep, elem, link)			\
	ES_RING_SPLICE_TAIL((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_CONCAT(h1, h2, elem, link) do {			\
	if (!ES_RING_EMPTY((h2), elem, link)) {			\
	    ES_RING_SPLICE_BEFORE(ES_RING_SENTINEL((h1), elem, link),	\
				  ES_RING_FIRST((h2)),			\
				  ES_RING_LAST((h2)), link);		\
	    ES_RING_INIT((h2), elem, link);				\
	}								\
    } while (0)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_PREPEND(h1, h2, elem, link) do {			\
	if (!ES_RING_EMPTY((h2), elem, link)) {			\
	    ES_RING_SPLICE_AFTER(ES_RING_SENTINEL((h1), elem, link),	\
				  ES_RING_FIRST((h2)),			\
				  ES_RING_LAST((h2)), link);		\
	    ES_RING_INIT((h2), elem, link);				\
	}								\
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_UNSPLICE(ep1, epN, link) do {				\
	ES_RING_NEXT(ES_RING_PREV((ep1), link), link) =		\
		     ES_RING_NEXT((epN), link);			\
	ES_RING_PREV(ES_RING_NEXT((epN), link), link) =		\
		     ES_RING_PREV((ep1), link);			\
    } while (0)
 ...
 * @param link The name of the ES_RING_ENTRY in the element struct
 */
#define ES_RING_REMOVE(ep, link)					\
    ES_RING_UNSPLICE((ep), (ep), link)
