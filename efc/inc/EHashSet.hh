...
public:
	 ...
	 */
	EHashSet() {
		map_ = new EHashMap<E, EObject*>(16);
	}
...
template<typename T>
class EHashSet<T*>: public EAbstractSet<T*> {
public:
	typedef T* E;
...
template<typename T>
class EHashSet<sp<T> >: public EAbstractSet<sp<T> > {
public:
	typedef sp<T> E;
	 ...
	 */
	EHashSet() {
		map_ = new EHashMap<E, sp<EObject> >(16);
	}
	 ...
	 */
	EHashSet(ECollection<E> *c) {
		map_ = new EHashMap<E, sp<EObject> >(
				EMath::max((int) (c->size() / .75f) + 1, 16));
		addAll(c);
	}
	...
	EHashSet(const EHashSet<E>& that) {
		EHashSet<E>* t = (EHashSet<E>*)&that;
		map_ = new EHashMap<E, sp<EObject> >(*t->map_);
	}
