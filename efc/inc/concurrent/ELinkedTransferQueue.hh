/*
 * ELinkedTransferQueue.hh
 *
 *  Created on: 2015-12-1
 *      Author: cxxjava@163.com
 */

#ifndef ELINKEDTRANSFERQUEUE_HH_
#define ELINKEDTRANSFERQUEUE_HH_
...
//@see: jdk1.8.0_65/src/java/util/concurrent/LinkedTransferQueue.java
//@see: http://bugs.java.com/bugdatabase/view_bug.do?bug_id=8043743
...
public:
	 ...
	 */
	ELinkedTransferQueue() : sweepVotes(0) {
	}
...
private:
	...
	static const boolean MP = true; //Runtime.getRuntime().availableProcessors() > 1;
     ...
     */
    static const int FRONT_SPINS   = 1 << 7;
     ...
     * power of two.
     */
    static const int CHAINED_SPINS = (unsigned)FRONT_SPINS >> 1;
     ...
     */
    static const int SWEEP_THRESHOLD = 32;

    /**
	 * Special value used to identify node
	 */
	static const int FORGET_ITEM = 1;
	static const int FORGET_NEXT = 2;
	...
	public:
		...
		boolean casNext(es_nullptr_t, sp<Node> val) {
			sp<Node> cmp;
			return atomic_compare_exchange(&next, &cmp, val);
		}
		 ...
		 */
		void forgetNext() {
			//@see: UNSAFE.putObject(this, nextOffset, this);
			sp<Node> n((Node*)FORGET_NEXT);
			atomic_store(&next, n);
		}
		...
		boolean isUnmatchedRequest() {
			return !isData && atomic_load(&item) == null;
		}
		 ...
		 *
		 */
		static sp<Node> getNext(sp<Node>& s) {
			sp<Node> n = atomic_load(&s->next);
			if (n == (Node*)FORGET_NEXT) {
				return s;
			}
			return n;
		}
	...
	private:
		...
		void advance(sp<Node> prev) {
			...
			sp<Node> r, b; // reset lastPred upon possible deletion of lastRet
			if ((r = lastRet) != null && !r->isMatched())
				lastPred = r;    // next lastPred is old lastRet
			else if ((b = lastPred) == null || b->isMatched())
				lastPred = null; // at start of list
			else {
				sp<Node> s, n;       // help with removal of lastPred.next
				while ((s = Node::getNext(b)) != null &&
					   s != b && s->isMatched() &&
					   (n = Node::getNext(s)) != null && n != s)
					b->casNext(s, n);
			}

			this->lastRet = prev;

			for (sp<Node> p = prev, s, n;;) {
				s = (p == null) ? atomic_load(&queue->head) : Node::getNext(p);
				if (s == null)
					break;
				else if (s == p) {
					p = null;
	...
	boolean casHead(es_nullptr_t, sp<Node> val) {
		sp<Node> cmp;
		return atomic_compare_exchange(&head, &cmp, val);
	}

	boolean casSweepVotes(int cmp, int val) {
		return EUnsafe::compareAndSwapInt(&sweepVotes, cmp, val);
	}

	/*
	 * Possible values for "how" argument in xfer method.
	 */
	static const int NOW   = 0; // for untimed poll, tryTransfer
	static const int ASYNC = 1; // for offer, put, add
	static const int SYNC  = 2; // for transfer, take
	static const int TIMED = 3; // for timed poll, tryTransfer
			...
			for (sp<Node> h = atomic_load(&head), p = h; p != null;) { // find & match first node
				...
				if (item != (E*)FORGET_ITEM && (item != null) == isData) { // unmatched
					...
					if (p->casItem(item, e)) { // match
						for (sp<Node> q = p; q != h;) {
							sp<Node> n = Node::getNext(q);  // update by 2 unless singleton
							if (atomic_load(&head) == h && casHead(h, n == null ? q : n)) {
								h->forgetNext();
								break;
							}                 // advance and retry
							if ((h = atomic_load(&head))   == null ||
								(q = Node::getNext(h)) == null || !q->isMatched())
								break;        // unless slack < 2
						}
						ELockSupport::unpark(p->waiter);
						return item;
					}
				}
				sp<Node> n = Node::getNext(p);
				p = (p != n) ? n : (h = atomic_load(&head)); // Use head if p offlist
			}

			if (how != NOW) {                 // No matches available
				if (s == null)
					s = new Node(e, haveData);
				sp<Node> pred = tryAppend(s, haveData);
				if (pred == null) {
	 ...
	 * predecessor
	 */
	sp<Node> tryAppend(sp<Node> s, boolean haveData) {
		for (sp<Node> t = atomic_load(&tail), p = t;;) {        // move p to last node and append
			sp<Node> n, u;                        // temps for reads of next & tail
			if (p == null && (p = atomic_load(&head)) == null) {
				if (casHead(null, s))
					return s;                 // initialize
			}
			else if (p->cannotPrecede(haveData))
				return null;                  // lost race vs opposite mode
			else if ((n = Node::getNext(p)) != null)    // not last; keep traversing
				p = p != t && t != (u = atomic_load(&tail)) ? (t = u) : // stale tail
					(p != n) ? n : null;      // restart if off list
			else if (!p->casNext(null, s))
				p = Node::getNext(p);                   // re-read on CAS failure
			else {
				if (p != t) {                 // update if slack now >= 2
					while ((atomic_load(&tail) != t || !casTail(t, s)) &&
						   (t = atomic_load(&tail))   != null &&
						   (s = Node::getNext(t)) != null && // advance and retry
						   (s = Node::getNext(s)) != null && s != t);
				}
				return p;
			}
		}
		...
		for (;;) {
			...
			if (spins < 0) {                  // establish spins at/near front
				if ((spins = spinsFor(pred, s->isData)) > 0)
					randomYields = EThreadLocalRandom::current();
			}
			else if (spins > 0) {             // spin
				--spins;
				if (randomYields->nextInt(CHAINED_SPINS) == 0)
					EThread::yield();           // occasionally yield
			}
				...
				if (nanos > 0L)
					ELockSupport::parkNanos(/*this, */nanos);
			}
			else {
	 ...
	 */
	static int spinsFor(sp<Node> pred, boolean haveData) {
		if (MP && pred != null) {
	 ...
	 */
	sp<Node> succ(sp<Node> p) {
		sp<Node> next = Node::getNext(p);
		return (p == next) ? atomic_load(&head) : next;
	}
	 ...
	 */
	sp<Node> firstOfMode(boolean isData) {
		for (sp<Node> p = atomic_load(&head); p != null; p = succ(p)) {
			if (!p->isMatched())
	 ...
	 */
	int countOfMode(boolean data) {
		int count = 0;
		for (sp<Node> p = atomic_load(&head); p != null; ) {
			if (!p->isMatched()) {
				if (p->isData != data)
					return 0;
				if (++count == EInteger::MAX_VALUE) // saturated
					break;
			}
			sp<Node> n = Node::getNext(p);
			if (n != p)
	 ...
	 */
	void unsplice(sp<Node> pred, sp<Node> s) {
		 ...
		 */
		if (pred != null && pred != s && Node::getNext(pred) == s) {
			sp<Node> n = Node::getNext(s);
			if (n == null ||
				...
				for (;;) {               // check if at, or could be, head
					...
					if (!h->isMatched())
						break;
					sp<Node> hn = Node::getNext(h);
					if (hn == null)
						return;          // now empty
					if (hn != h && casHead(h, hn))
						h->forgetNext();  // advance head
				}
				if (Node::getNext(pred) != pred && Node::getNext(s) != s) { // recheck if offlist
	 ...
	 */
	void sweep() {
		for (sp<Node> p = atomic_load(&head), s, n; p != null && (s = Node::getNext(p)) != null; ) {
			if (!s->isMatched())
				// Unmatched nodes are never self-linked
				p = s;
			else if ((n = Node::getNext(s)) == null) // trailing node is pinned
				break;
			else if (s == n)    // stale
		...
		if (e != null) {
			for (sp<Node> pred, p = atomic_load(&head); p != null; ) {
				...
				if (p->isData) {
					...
					break;
				pred = p;
				if ((p = Node::getNext(p)) == pred) { // stale
					pred = null;
					p = atomic_load(&head);
				}
			}
...
#endif /* ELINKEDTRANSFERQUEUE_HH_ */
